...
#define P 1 /* If match length <= P then output one character */ #define N (1 << EI) /* buffer size */
...
void encode(void) {
    ...
    for (i = N - F; i < N * 2; i++) {
        ...
        buffer[i] = c;  textcount++;
			...
			while (r < bufferend) {
				￼f1 = (F <= bufferend - r) ? F : bufferend - r; //makes sure we do not do bufferoverflow - might have used min()
				￼x = 0;
				y = 1;
